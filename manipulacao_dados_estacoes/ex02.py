...
lat, lon, temp = np.loadtxt('../dados/texto/data_station.dat', usecols=(1,2,3), unpack=True)

# Interpolando os dados de estação para a grade regularmente espaçada.
nPoints = 1000  # Número de pontos a ser considerado na interpolação.
loni = np.linspace(np.min(lon), np.max(lon), nPoints)
lati = np.linspace(np.min(lat), np.max(lat), nPoints)
lonf, latf = np.meshgrid(loni, lati)
gridmap = griddata((lon, lat), temp, (lonf, latf), method='cubic')
...
states = NaturalEarthFeature(category='cultural', scale='10m', facecolor='none', name='admin_1_states_provinces')
...
plot = plt.contourf(lonf, latf, gridmap, levels=100, cmap='RdYlGn_r')
...
plt.tight_layout()
plt.savefig('ex02.tif', bbox_inches='tight')